...
export default function RegisterPage() {
  ...
  const [role, setRole] = useState<Role>(Role.STORE_KEEPER);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await registerMutation({
        variables: {
          registerInput: { name: name.trim(), email, password, role },
        },
      });

      if (data?.register) {
        authLogin(data.register.accessToken, data.register.user);
      }
    } catch (err: unknown) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() => setRole(Role.STORE_KEEPER)}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-medium rounded-lg transition-colors flex items-center justify-center mt-2"
            >
              {loading ? (
                <>
